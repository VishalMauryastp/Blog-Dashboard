<div class="grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 gap-x-8 gap-y-20 text-center mt-32">
    @foreach($team as $member)
        <div class="bg-gray-200 relative rounded">
            <img src="{{ asset('storage/' . $member->image) }}" class="w-32 h-32 rounded-full inline-block -mt-12" />
            <div class="py-6">
                <h4 class="text-base font-semibold">{{ $member->name }}</h4>
                <p class="text-xs mt-1">{{ $member->title }}</p>
